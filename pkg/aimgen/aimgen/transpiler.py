...
class Transpiler(TranspilerBase):
    ...
    def parse_enum(self, node):
        ...
        #spelling = node.spelling if node.spelling else node.type.spelling
        #self.scope(f'py::enum_<{spelling}>({self.module}, "{self.format_type(spelling)}", py::arithmetic())')
        #
        self.scope(f'py::enum_<{self.spell(node)}>({self.module}, "{self.format_type(self.spelling(node))}", py::arithmetic())')
        self.indent()
    ...
    def parse_field(self, node, cls):
        pyname = self.format_attribute(self.spelling(node))
        ...
        if self.is_property_mappable(node):
            if self.is_property_readonly(node):
                ...
                self.scope(f'.def_readonly("{pyname}", &{cname})')
            else:
                #self.scope(f'{self.module_(cls)}.def_readwrite("{pyname}", &{cname})')
                self.scope(f'.def_readwrite("{pyname}", &{cname})')
    ...
    def parse_struct(self, node):
        if self.is_class_mappable(node):
            ...
            pyname = self.format_type(spelling)
            ...
            self.scope(f'.def(py::init<>([](){{ return {clsname}(); }}))')
